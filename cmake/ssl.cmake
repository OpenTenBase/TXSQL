...
# build bundled openssl, version:1.1.1k
MACRO (MYSQL_USE_BUNDLED_SSL)
  SET(CDB_SSL_SOURCE_DIR "${CMAKE_SOURCE_DIR}/extra/ssl/openssl-1.1.1k")
  SET(CDB_SSL_BINARY_DIR "${CMAKE_BINARY_DIR}/${CMAKE_CFG_INTDIR}/extra/ssl/openssl-1.1.1k/build")
  SET(CDB_SSL_INSTALL_DIR "${CMAKE_BINARY_DIR}/${CMAKE_CFG_INTDIR}/extra/ssl/openssl-1.1.1k/install")
  SET(CDB_SSL_CONFIGURE_OPTS "CC=${CMAKE_C_COMPILER} ${CMAKE_C_COMPILER_ARG1}")
  LIST(APPEND CDB_SSL_CONFIGURE_OPTS "CFLAGS=${CMAKE_C_FLAGS}")
  LIST(APPEND CDB_SSL_CONFIGURE_OPTS "CXX=${CMAKE_CXX_COMPILER} ${CMAKE_CXX_COMPILER_ARG1}")
  LIST(APPEND CDB_SSL_CONFIGURE_OPTS "CXXFLAGS=${CMAKE_CXX_FLAGS}")
  LIST(APPEND CDB_SSL_CONFIGURE_OPTS "--prefix=${CDB_SSL_INSTALL_DIR}")

  IF (CMAKE_BUILD_TYPE MATCHES "Debug" AND NOT APPLE) # see the comment in CMakeLists.txt
    LIST(APPEND CDB_SSL_CONFIGURE_OPTS --debug)
  ENDIF()
  ...
  # config  and build openssl
  MAKE_DIRECTORY(${CDB_SSL_BINARY_DIR})
  MAKE_DIRECTORY(${CDB_SSL_INSTALL_DIR})
  EXECUTE_PROCESS(COMMAND ${CDB_SSL_SOURCE_DIR}/config ${CDB_SSL_CONFIGURE_OPTS} WORKING_DIRECTORY ${CDB_SSL_BINARY_DIR})
  CMAKE_HOST_SYSTEM_INFORMATION(RESULT Ncpu QUERY NUMBER_OF_PHYSICAL_CORES)
  EXECUTE_PROCESS(COMMAND ${MAKE_COMMAND} "-j${Ncpu}" "--silent" WORKING_DIRECTORY ${CDB_SSL_BINARY_DIR})
  EXECUTE_PROCESS(COMMAND ${MAKE_COMMAND} "install_dev" "-j${Ncpu}" "--silent" WORKING_DIRECTORY ${CDB_SSL_BINARY_DIR})
  SET(WITH_SSL ${CDB_SSL_INSTALL_DIR})
...
MACRO (MYSQL_CHECK_SSL)
  ...
  IF(WITH_SSL STREQUAL "bundled" OR trybundled)
