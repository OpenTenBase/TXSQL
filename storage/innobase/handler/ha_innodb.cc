...
ha_innobase::ha_innobase(handlerton *hton, TABLE_SHARE *table_arg)
    : handler(hton, table_arg),
      ...
      m_mysql_has_locked(),
      m_sampling_method(enum_sampling_method::NONE) {}
...
int ha_innobase::index_init_with_num(uint keynr, uint64_t n_wanted, bool sorted) {
  DBUG_TRACE;

  return change_active_index(keynr, n_wanted);
}
...
int ha_innobase::index_end(void) {
  ...
  if (m_prebuilt->key_extracter) {
    delete m_prebuilt->key_extracter;
...
int ha_innobase::change_active_index(
    uint keynr, uint64_t n_wanted) /*!< in: use this index; MAX_KEY means always clustered
