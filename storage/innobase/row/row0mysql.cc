...
byte *row_compress_column(const byte *data, ulint *len, ulint lenlen,
  ...
  switch (algorithm_type) {
    ...
    case ZSTD_COL_COMP:
      ...
      break;
    default:
      ib::fatal() << "unsupported 'algorithm' value in the compressed BLOB header\n";
  }
...
const byte *row_decompress_column(const byte *data, ulint *len,
  ...
  switch (alg) {
    case ZLIB_COL_COMP:
      ...
      break;
    case LZ4_COL_COMP:
      err = row_decompress_column_lz4(buf, &buf_len, const_cast<byte *>(data), comp_len);
      break;
    case ZSTD_COL_COMP:
      err = row_decompress_column_zstd(buf, &buf_len, const_cast<byte *>(data), comp_len);
      break;
    default:
      ib::fatal() << "unsupported 'algorithm' value in the compressed BLOB header\n";
  }
  ...
  ib::fatal() << "failed to decompress column.\n";
  ...
  for (i = 0; i < prebuilt->n_template; i++) {
    ...
    if (is_multi_val) {
      ...
      row_mysql_store_col_in_innobase_format(
          ...
          dict_table_is_comp(prebuilt->table), templ->is_compressed,
          templ->col_comp_algorithm, prebuilt);
...
static dberr_t row_del_upd_for_mysql_using_cursor(const byte *mysql_rec,
  ...
  row_upd_store_row(prebuilt->trx, node, NULL, NULL, NULL);
