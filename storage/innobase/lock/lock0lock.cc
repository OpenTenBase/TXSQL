    ...
    bool mc_enable)  /*!< in: GTS mode readview enable or not */
{
  ...
  if (srv_read_only_mode || index->table->is_temporary()) {
    ut_ad(view == nullptr || (!view->is_open()) || index->table->is_temporary());
    return (true);
  }
    ...
    ulint n_cells) /*!< in: number of slots in lock hash table */
{
  ...
  lock_sys->hot_update_hash = ut::new_<hash_table_t>(n_cells);
...
void lock_sys_close(void) {
  ...
  ut::delete_(lock_sys->hot_update_hash);
...
static inline void lock_set_lock_and_trx_wait(lock_t *lock) {
  ...
  ib::info() << "trx " << trx->id << " was set wait_lock = "
    << trx->lock.wait_lock << ".";

  if (trx->hot_update_status == HOT_UPDATE_STATUS_WAITING) {
    ib::info() << "trx " << trx->id
      << " set wait_lock incorrectly.";
  }
#endif
}
...
/** Checks if some transaction has an implicit x-lock on a record in a secondary
 ...
 is still active. */
static trx_t *lock_sec_rec_some_has_impl(trx_t *caller_trx, const rec_t *rec, dict_index_t *index,
                                         const ulint *offsets) {
  trx_t *trx;
  trx_id_t max_trx_id;
  ...
  if (!recv_recovery_is_on() && !trx_sys->is_trx_id_possible_active(max_trx_id)) {
    trx = nullptr;
...
struct lock_rec_other_trx_holds_expl_callback_arg {
  trx_t *impl_trx;
  ulint precise_mode;
  const buf_block_t *block;
  ulint heap_no;
...
static bool lock_rec_other_trx_holds_expl_callback(rw_trx_hash_element_t* elem,
                                                   lock_rec_other_trx_holds_expl_callback_arg *arg) {
  bool holds = false;
  ...
  if (trx) {
    const lock_t *expl_lock = lock_rec_has_expl(arg->precise_mode, arg->block,
                                                arg->heap_no, trx);
    if (expl_lock && expl_lock->trx != arg->impl_trx) {
...
static bool lock_rec_other_trx_holds_expl(ulint precise_mode, const trx_t *trx,
  ...
  locksys::Global_exclusive_latch_guard guard{UT_LOCATION_HERE};
  /* If trx_sys->find() returns non-null impl_trx it only means that impl_trx
...
*/
bool lock_get_hot_row_update_stats(std::vector<ulint> & hot_row_space_id_col,
                   ...
                   std::vector<ulint> & hot_row_waiting_trxs_col)
{
  ...
  for (ulint i = 0; i < hash_get_n_cells(lock_sys->hot_update_hash); ++i) {
    const hot_update_item_t*  item;

    for ( item = static_cast<const hot_update_item_t*>(
          hash_get_first(lock_sys->hot_update_hash, i));
          item != NULL; ) {
      hot_row_space_id_col.push_back(item->m_rec_id.get_page_id().space());
      hot_row_page_no_col.push_back(item->m_rec_id.get_page_id().page_no());
      hot_row_heap_no_col.push_back(item->m_rec_id.m_heap_no);
      hot_row_running_trxs_col.push_back(item->n_running);
      hot_row_waiting_trxs_col.push_back(item->waiting_updates->size());
...
@param[in]  n_running  running trx counter. */
static
void
...
static void lock_rec_grant_by_heap_no(lock_t *in_lock, ulint heap_no) {
  ...
  ulint total_waiting = waiting.size() + low_priority_heavier.size() +
         low_priority_light.size();
  ...
  if (srv_hot_update_detect && in_lock->trx->is_point_update
      ...
      && total_waiting > 32) {
    lock_clust_add_or_update_hot_update_item(
      const_cast<trx_t*>(in_trx), rec_id, total_waiting);
  }
  ...
  if (srv_hot_update_detect && in_lock->trx->is_point_update
      ...
      && waiting.size() > 32) {
    lock_clust_add_or_update_hot_update_item(
      const_cast<trx_t*>(in_trx), rec_id, waiting.size());
  }
...
This function makes sure the corresponding hot update item point to the
updated location.
@param[in]  old_block    old block
@param[in]  old_heap_no  old heap no of rec
@param[in]  new_block    new block
@param[in]  new_heap_no  new heap no of rec */
static
void
lock_rec_move_hot_update(
  const buf_block_t*  old_block,
  ulint               old_heap_no,
  const buf_block_t*  new_block,
  ulint               new_heap_no)
{
  RecID  rec_id(old_block, old_heap_no);
...
/** Notify a hot update waiting trx to continue.
@param[in]  trx    trx object */
static
bool
lock_rec_grant_hot_update_low(trx_t*  trx, bool owns_trx_mutex)
{
  que_thr_t*  thr;
  ...
  if (thr != NULL) {
    ...
    ib::info() << "trx " << trx->id
      << " has no waiting slot yet since thr is null.";
  }
#endif
  if (!owns_trx_mutex) {
    trx_mutex_exit(trx);
  ...
  ulint       heap_no)
{
  ...
  if (!item->waiting_updates->empty()) {
    hot_update_t  next_update =
      item->waiting_updates->front();
    for (ulint i = item->n_running;
         i <  srv_max_concurrent_hot_update; i++) {

      item->n_running++;

      if (lock_rec_grant_hot_update_low(next_update.m_trx, true)) {
        ...
        // waiting_updates queue timed-out during the wait and
        // started to run. In this case, we remove the next_update 
        // from the waiting_updates queue directly.
        item->waiting_updates->pop_front();
      }
  ...
  if (item->n_running == 0 && item->waiting_updates->empty()) {
    ut::delete_(item->waiting_updates);
    HASH_DELETE(hot_update_item_t, hash,
        lock_sys->hot_update_hash,
        item->m_rec_id.hash_value(),
        item);
#ifdef UNIV_DEBUG_HOT_UPDATE
    ib::info() << "hot update item has "
      << item->n_running << " running."
      << " The waiting_updates is empty, "
      <<"removing hot_update_item.";
#endif
    my_free(item);
  }
...
lock_sys_reset_hot_update()
{
  ...
  for (ulint i = 0; i < hash_get_n_cells(lock_sys->hot_update_hash); i++) {
    ...
    for (item = static_cast<const hot_update_item_t*>(
      ...
      for (ulint j = 0; j < item->waiting_updates->size();
           j++) {
...
#ifdef UNIV_DEBUG_HOT_UPDATE
    if (hot_update_granted == 0) {
      ib::info() << "trx " << in_lock->trx->id
        << " release the lock but not notify any other trxs.";
    }
#endif
  }
...
/** Resets the hot update item for a single record. Releases transactions waiting
for hot update here. 
@param[in]	block		buffer block containing the record
@param[in]	heap_no heap number of record  */
static
void
lock_rec_reset_and_release_hot_update(
	const buf_block_t*	block,
	ulint			          heap_no)
{
  RecID	rec_id(block, heap_no);
  ...
  if (item) {
    ...
    HASH_DELETE(hot_update_item_t, hash, lock_sys->hot_update_hash,
          item->m_rec_id.hash_value(), item);
#ifdef UNIV_DEBUG_HOT_UPDATE
    ib::info() << "removed hot update item from ("
      << rec_id.get_page_id().space() << ","
      << rec_id.get_page_id().page_no() << ","
      << rec_id.m_heap_no << ").";
#endif
    /* For test add new item */
    DBUG_EXECUTE_IF("hot_update_with_delete",
        ib::info() << "removed hot update item from ("
          << rec_id.get_page_id().space()<< ","
          << rec_id.get_page_id().page_no() << ","
          << rec_id.m_heap_no << ").";);
                                 ...
                                 which gives the locks */
{
  ...
  for (lock = lock_rec_get_first(lock_hash, donator, donator_heap_no);
    ...
    /* If hot update detect is enabled, update
    the item. */
    if (srv_hot_update_detect
        && lock->index->is_clustered()) {
      lock_rec_move_hot_update(donator,
              donator_heap_no,
              receiver,
              receiver_heap_no);
    }
    ...
    for (auto lock : old_locks) {
      ...
      for (;;) {
        ...
        if (old_heap_no < lock->rec_lock.n_bits &&
          ...
          if (srv_hot_update_detect
              && lock->index->is_clustered()) {
            lock_rec_move_hot_update(block,
                    old_heap_no,
                    block,
                    new_heap_no);
          }
...
static void lock_rec_release(lock_t *lock, ulint heap_no) {
  ...
  if (srv_hot_update_detect
    ...
    if (item != NULL && !item->waiting_updates->empty()) {
      ib::info() << " release lock but waiting update is not empty.";
    }
    mutex_exit(&lock_sys->hot_update_mutex);
  }
#endif
...
struct lock_remove_recovered_trx_record_locks_callback_arg {
  dict_table_t *table;
  ulint n_recovered_trx;
};

static bool lock_remove_recovered_trx_record_locks_callback(
    rw_trx_hash_element_t *elem,
    lock_remove_recovered_trx_record_locks_callback_arg *arg) {

  mutex_enter(&elem->mutex);
  trx_t *trx = elem->trx;
  if (trx) {
    /* FIXME larry
    Currently, assert_trx_in_rw_hash will invoke hash.find,
    which will re-enter the rw_trx_hash_element_t.mutex.*/
    // assert_trx_in_rw_hash(trx);
    if (trx->is_recovered) {
      /* We need trx->mutex to iterate over trx->lock.trx_lock and it is needed by
                         ...
                         table itself */
{
  ...
  lock_remove_recovered_trx_record_locks_callback_arg arg = {table, 0};
  TRX_HASH_ITERATE(nullptr, lock_remove_recovered_trx_record_locks_callback, &arg);
...
static void lock_trx_print_locks(FILE *file, trx_t *trx) {
  /* We require exclusive access to lock_sys */
  ut_ad(locksys::owns_exclusive_global_latch());
  ...
  for (lock_t *lock = UT_LIST_GET_FIRST(trx->lock.trx_locks);
    ...
    if (count++ >= 10) {
      fprintf(file, "10 LOCKS PRINTED FOR THIS TRX: SUPPRESSING FURTHER PRINTS\n");
      break;
    }
  }
}

static bool lock_trx_print_locks_callback(rw_trx_hash_element_t *elem,
                                          FILE *file) {
  mutex_enter(&elem->mutex);
  trx_t *trx = elem->trx;
  ...
  if (trx) {
    ...
    if (srv_print_innodb_monitor) {
      trx->n_ref++;
      // release elem mutex, since trx->n_ref++,
      // trx object will not be freed because we check n_ref when to free trx.
      // elem object is protected by pins, because we are doing rw_trx_hash iterating now.
      mutex_exit(&elem->mutex); 
...
static bool lock_validate_table_locks_callback(rw_trx_hash_element_t *elem, void *arg) {
  mutex_enter(&elem->mutex);
  trx_t *trx = elem->trx;
  if (trx) {
    check_trx_state(trx);
    ...
    ut_ad(locksys::owns_exclusive_global_latch());
    for (const lock_t *lock = UT_LIST_GET_FIRST(trx->lock.trx_locks); lock != nullptr;
         lock = UT_LIST_GET_NEXT(trx_locks, lock)) {
...
bool lock_validate() {
  ...
  page_addr_set pages;
  {
    /* lock_validate_table_locks_callback needs exclusive global latch, and we will
    inspect record locks from all shards */
    locksys::Global_exclusive_latch_guard guard{UT_LOCATION_HERE};

    TRX_HASH_ITERATE(nullptr, lock_validate_table_locks_callback, nullptr);
...
dberr_t
lock_clust_check_hot_row_update(
	trx_t       *trx,
	const rec_t *rec)
{
  hot_update_item_t*  item;
...
#ifdef UNIV_DEBUG_HOT_UPDATE
    ib::info() << "item has " << item->n_running << " running "
      "waiting size "
      << item->waiting_updates->size() << ".";
#endif
  }
...
#ifdef UNIV_DEBUG_HOT_UPDATE
  ib::info() << "trx " << trx->id << " continue to run after waiting.";
#endif
  return (DB_SUCCESS);
}
	...
	ulint		        n_running)
{
  ...
  /* If found an existed item, return. */
  ...
  item->m_rec_id.get_page_id_non_const().reset(rec_id.get_page_id().space(),
                                               rec_id.get_page_id().page_no());
  item->m_rec_id.m_heap_no = rec_id.m_heap_no;
  item->m_rec_id.m_hash_value = rec_id.m_hash_value;
  item->n_running = n_running;
  item->waiting_updates =
    ut::new_withkey<hot_update_queue>(UT_NEW_THIS_FILE_PSI_KEY);

  ulint    key = item->m_rec_id.hash_value();
...
#endif
  /* For test add new item */
  DBUG_EXECUTE_IF("hot_update_add_new_item",
      ib::info() << "trx " << trx->id
        << " added a new hot update item.";);
...
struct lock_table_locks_lookup_callback_arg {
  const dict_table_t *table;
  lock_t *lock_found;
};

/** Scans all locks of all transactions in the rw_trx_hash searching for any
lock (table or rec) against the table. */

static bool lock_table_locks_lookup_callback(
    rw_trx_hash_element_t *elem,
    lock_table_locks_lookup_callback_arg *arg) {
  ...
  if (trx) {
    ...
    for (lock = UT_LIST_GET_FIRST(trx->lock.trx_locks);
      ...
      ut_a(lock->trx == trx);
      if (lock_get_type_low(lock) == LOCK_REC) {
        ut_ad(!dict_index_is_online_ddl(lock->index) || lock->index->is_clustered());
        if (lock->index->table == arg->table) {
          found = true;
          arg->lock_found = lock;
        }
      } else if (lock->tab_lock.table == arg->table) {
        found = true;
        arg->lock_found = lock;
      }
    } // end for
...
bool lock_table_has_locks(const dict_table_t *table) {
  ...
  if (!has_locks) {
    locksys::Global_exclusive_latch_guard guard{UT_LOCATION_HERE};
    lock_table_locks_lookup_callback_arg arg = {table, nullptr};
    TRX_HASH_ITERATE(nullptr, lock_table_locks_lookup_callback, &arg);
    ut_ad(arg.lock_found == nullptr);
  }
#endif /* UNIV_DEBUG */
