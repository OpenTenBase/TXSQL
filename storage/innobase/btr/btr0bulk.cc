...
dberr_t PageBulk::init() {
  ...
  m_heap = mem_heap_create(1000, UT_LOCATION_HERE);
  ...
  if (!dict_index_is_online_ddl(m_index)) {
    mtr_x_lock(dict_index_get_lock(m_index), mtr, UT_LOCATION_HERE);
  }
  ...
  if (m_page_no == FIL_NULL) {
    ...
    new_block = btr_block_get(page_id, page_size, RW_X_LATCH,
...
void PageBulk::insert(const rec_t *rec, ulint *offsets) {
  ...
  if (!page_rec_is_infimum(m_cur_rec)) {
    ...
    ulint *old_offsets =
        rec_get_offsets(old_rec, m_index, nullptr, ULINT_UNDEFINED,
        UT_LOCATION_HERE, &m_heap);
...
bool PageBulk::compress(bool leftmost) {
  ut_ad(!m_modified);
  ut_ad(m_page_zip != nullptr);

  return (
      page_zip_compress(m_page_zip, m_page, m_index, page_zip_level, m_mtr, leftmost));
}
...
PageBulk::SplitPoint PageBulk::getSplitRec() {
  ...
  do {
    ...
    offsets =
        rec_get_offsets(rec, m_index, offsets, ULINT_UNDEFINED,
                        UT_LOCATION_HERE, &(m_heap));
...
void PageBulk::latch() {
  ...
  if (!dict_index_is_online_ddl(m_index)) {
    mtr_x_lock(dict_index_get_lock(m_index), m_mtr, UT_LOCATION_HERE);
  }
...
dberr_t BtrBulk::pageCommit(PageBulk *page_bulk, PageBulk *next_page_bulk,
  ...
  bool leftmost = (m_as_subtree == nullptr)
                  || (m_as_subtree->is_leftmost_subtree);
  /* Compress page if it's a compressed table. */
  if (page_bulk->isTableCompressed() && !page_bulk->compress(leftmost)) {
...
BtrBulk::BtrBulk(dict_index_t *index, trx_id_t trx_id, Flush_observer *observer, AsSubtree *as_subtree)
    : m_as_subtree(as_subtree),
      m_index(index),
      m_trx_id(trx_id),
      m_root_level(0),
...
dberr_t BtrBulk::init() {
  ...
  m_page_bulks = ut::new_withkey<page_bulk_vector>(UT_NEW_THIS_FILE_PSI_KEY);
...
dberr_t BtrBulk::prepareSpace(PageBulk *&page_bulk, ulint level,
  ...
  PageBulk *sibling_page_bulk = ut::new_withkey<PageBulk>(
        UT_NEW_THIS_FILE_PSI_KEY, m_index, m_trx_id,
...
dberr_t BtrBulk::insert(dtuple_t *tuple, ulint level) {
  ...
  if (level + 1 > m_page_bulks->size()) {
    PageBulk *new_page_bulk = ut::new_withkey<PageBulk>(
        UT_NEW_THIS_FILE_PSI_KEY, m_index, m_trx_id,
        FIL_NULL, level, m_flush_observer);
...
dberr_t BtrBulk::finishAllPageBulks(dberr_t err, page_no_t &last_page_no) {
  ...
  for (ulint level = 0; level <= m_root_level; level++) {
    ...
    /** if work as subtree, then delete page bulks later in destroy_page_bulks(),
    because we may use them when merge subtrees. */
    if (m_as_subtree == nullptr) {
      ut::delete_(page_bulk);
    }
  }
...
void BtrBulk::destroy_page_bulks() {
  /** delete the object in m_page_bulks.
  m_page_bulks will destroy in destructor.
  Actually we may also destroy m_page_bulks here. */

  /** if not as subtree, we will not deter deletion */
  ut_a(m_as_subtree);
  for (auto p : *m_page_bulks) {
    ut::delete_(p);
  }
}
...
dberr_t BtrBulk::finish(dberr_t err) {
  ...
  if (err == DB_SUCCESS && m_as_subtree == nullptr) {
    ...
    buf_block_t *last_block =
        btr_block_get(last_page_id, page_size, RW_X_LATCH,
                      UT_LOCATION_HERE, m_index, &mtr);
...
dberr_t BtrBulk::raise_to_level(ulint max_level) {
  ut_a(m_root_level <= max_level);
  ut_a(m_as_subtree);

  if (m_root_level == max_level) {
    return DB_SUCCESS;
  }
...
page_no_t BtrBulk::get_subtree_root_page_no() const {
  PageBulk *cur_root_page_bulk = m_page_bulks->at(m_root_level);
  return cur_root_page_bulk->getPageNo();
}
