...
class PageBulk : private ut::Non_copyable {
 public:
  ...
  bool compress(bool leftmost) MY_ATTRIBUTE((warn_unused_result));
...
struct AsSubtree {
  bool is_leftmost_subtree;
};

class BtrBulk {
 public:
  using page_bulk_vector = std::vector<PageBulk *, ut::allocator<PageBulk *>>;

  /** Constructor
  @param[in]	index		B-tree index
  @param[in]	trx_id		transaction id
  @param[in]	observer	flush observer */
  BtrBulk(dict_index_t *index, trx_id_t trx_id, Flush_observer *observer, AsSubtree *as_subtree = nullptr);
  ...
  /** Raise this subtree to max_level */
  dberr_t raise_to_level(ulint max_level);

  /** Get root page_no as subtree */
  page_no_t get_subtree_root_page_no() const;

  /** Get root level */
  ulint get_root_level() const { return m_root_level; }
