...
static pthread_key_t THD_RPL_SPEED_LIMIT;

/* variables */
bool rpl_speed_limit_enabled;
...
/** Total sleep time of dump thread */
std::atomic<long long> rpl_speed_limit_sleep_time(0);

/** Sleep times of speed limit for dump threads */
std::atomic<long long> rpl_speed_limit_sleep_count(0);

/** Total amount of data sent by dump thread, unit byte */
std::atomic<long long> rpl_speed_limit_bytes(0);

/** Actual replication speed bandwidth */
...
RplSpeedLimit::RplSpeedLimit(THD *thd) : m_flags(0), m_thd(thd) {
  mysql_mutex_init(key_ss_mutex_Speed_limit_mutex_, &m_mutex,
                   MY_MUTEX_INIT_FAST);
}

RplSpeedLimit::~RplSpeedLimit() { mysql_mutex_destroy(&m_mutex); }

/** Update speed limit of current slave when adding or removing slaves */
void RplSpeedLimit::updateSpeed(bool reset_token, uint slave_cnt) {
  ...
  if (m_bak_speed < MIN_SPEED) m_bak_speed = MIN_SPEED;
  /* total bytes of one second */
  m_bak_max_token =
      m_bak_speed * rpl_speed_limit_max_token_ratio * 10;  // 10 = 1000ms /100%
  m_flags |= kUpdateBucket;
  if (reset_token) m_flags |= kResetToken;
...
void *speed_monitor_handler(void *) {
  ...
  while (speed_monitor_thread_enabled) {
    ...
    assert(sleep_time % max_sleep_time == 0);

    while (sleep_time >= max_sleep_time) {
      sleep_ms(max_sleep_time);
      sleep_time -= max_sleep_time;

      if (!speed_monitor_thread_enabled) break;
    }

    assert(sleep_time == 0 || !speed_monitor_thread_enabled);
  }
...
RplSpeedMonitor::RplSpeedMonitor() : m_enabled(false) {}
...
void RplSpeedMonitor::init() {
  ...
  mysql_mutex_init(key_ss_mutex_Speed_monitor_mutex_, &m_mutex,
                   MY_MUTEX_INIT_FAST);

  pthread_key_create(&THD_RPL_SPEED_LIMIT, NULL);

  if (mysql_thread_create(PSI_NOT_INSTRUMENTED, &m_thread, NULL,
                          speed_monitor_handler, NULL)) {
...
void RplSpeedMonitor::updateConf(bool need_lock, bool reset_token) {
  ...
  if (need_lock) mysql_mutex_unlock(&m_mutex);

  function_exit(kWho);
}

/** Update rpl_speed_limit_clients status */
...
bool RplSpeedMonitor::addSlave(THD *thd) {
  ...
  if (!m_enabled) return function_exit(kWho, false);
...
void RplSpeedMonitor::removeSlave(THD *thd) {
  ...
  RplSpeedLimit *info =
      (RplSpeedLimit *)pthread_getspecific(THD_RPL_SPEED_LIMIT);
  if (info) {
    mysql_mutex_lock(&m_mutex);
    ...
    if (iter != m_lists->end()) m_lists->erase(iter);
...
/**
  Core function for controlling replication speed. If there is
  no enough token for the sending package, sleep for a while to
  get enough token.
*/
bool RplSpeedMonitor::controlSpeed(THD *thd, ulong len) {
  if (!m_enabled) return false;

  RplSpeedLimit *info_ptr =
      (RplSpeedLimit *)pthread_getspecific(THD_RPL_SPEED_LIMIT);
  ...
  if (trace_level_ & kTraceDetail)
    sql_print_information(
        "server_id: %u >> token: %lld, max_token: %lld, speed: %lld, len: %lu",
        thd->server_id, bucket->token, bucket->max_token, bucket->speed, len);

  assert(bucket->token <= bucket->max_token);

  /* fast path */
  if (bucket->token >= (int64)len) {
    bucket->token -= len;
    rpl_speed_limit_bytes += len;
    return false;
  }
  ...
  do {
    ...
    if (unlikely(now < bucket->last_tick)) bucket->last_tick = now;
    ...
    rpl_speed_limit_sleep_time += sleep_time;
    ++rpl_speed_limit_sleep_count;
  ...
  /*
     ...
     never get it.
  */
  if (bucket->token > bucket->max_token) bucket->token = bucket->max_token;

  return false;
}

/** Test for speed limit */
void RplSpeedMonitor::run_test(THD *thd, ulong run_time) {
  ...
  sql_print_information(
      "send len: %lu, total len: %lld bytes, time used:"
      "%lu sec(expect: %lu K/ps, actual: %lu K/ps)",
      send_len, total_len, run_time, rpl_speed_limit_slave_bandwidth,
