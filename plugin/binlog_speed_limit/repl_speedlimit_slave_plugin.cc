...
int repl_semi_slave_io_start(Binlog_relay_IO_param *) {
  THD *thd = current_thd;
  if (speed_monitor.addSlave(thd)) return 1;

  return 0;
}

int repl_semi_slave_io_end(Binlog_relay_IO_param *) {
  THD *thd = current_thd;
 ...
 */
int repl_semi_slave_read_event(Binlog_relay_IO_param *, const char *,
                               unsigned long len, const char **,
                               unsigned long *) {
  THD *thd = current_thd;
  if (speed_monitor.controlSpeed(thd, len)) return 1;
...
SHOW_VAR repl_speed_limit_status_vars[] = {
    {"repl_speed_limit_slave_clients", (char *)&SHOW_FNAME(clients), SHOW_FUNC,
     SHOW_SCOPE_GLOBAL},
    {"repl_speed_limit_slave_sleep_time", (char *)&rpl_speed_limit_sleep_time,
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"repl_speed_limit_slave_sleep_count", (char *)&rpl_speed_limit_sleep_count,
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"repl_speed_limit_slave_bytes_received", (char *)&rpl_speed_limit_bytes,
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"repl_speed_limit_slave_bandwidth", (char *)&rpl_speed_limit_bandwidth,
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
