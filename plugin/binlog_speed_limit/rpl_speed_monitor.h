...
extern bool rpl_speed_limit_enabled;
...
extern std::atomic<long long> rpl_speed_limit_sleep_time;

extern std::atomic<long long> rpl_speed_limit_sleep_count;

extern std::atomic<long long> rpl_speed_limit_bytes;
...
/**
  This struct is used to hold token. Packages can be sent only if the
  sender get enough token. If there are no enough token available,
  sender has to sleep for a while.
*/
struct token_bucket {
...
/**
  This Class is specificd to each slave to update the
  information related to token.
*/
class RplSpeedLimit {
public:
...
/**
  This class acts as a monitor to manage all slaves
  replication speed.
*/
...
public:
  ...
  /** Core function for controlling replication speed */
  bool controlSpeed(THD* thd, ulong len);

private:
  /** Mutex to protect m_lists and m_thread */
  mysql_mutex_t m_mutex;

  /** Vector to hold slave speedlimit infos */
