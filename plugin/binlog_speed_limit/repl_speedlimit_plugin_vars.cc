...
static PSI_mutex_info all_speedlimit_mutexes[] = {
    {&key_ss_mutex_Speed_monitor_mutex_, "Speed_monitor::m_mutex", 0, 0,
     PSI_DOCUMENT_ME},
    {&key_ss_mutex_Speed_limit_mutex_, "Speed_limit::m_mutex", 0, 0,
     PSI_DOCUMENT_ME}};
...
void init_psi_keys(void) {
  ...
  count = static_cast<int>(array_elements(all_speedlimit_mutexes));
  mysql_mutex_register(category, all_speedlimit_mutexes, count);
}
...
static TYPELIB rpl_speed_limit_mode_typelib = {
    ...
    rpl_speed_limit_mode_names, NULL};

static void fix_rpl_speed_limit_mode(MYSQL_THD thd, SYS_VAR *var, void *ptr,
                                     const void *val);

static void fix_rpl_speed_limit_enabled(MYSQL_THD thd, SYS_VAR *var, void *ptr,
                                        const void *val);

static void fix_rpl_speed_limit_slave_bandwidth(MYSQL_THD thd, SYS_VAR *var,
                                                void *ptr, const void *val);

static void fix_rpl_speed_limit_total_bandwidth(MYSQL_THD thd, SYS_VAR *var,
                                                void *ptr, const void *val);

static void fix_rpl_speed_limit_max_token_ratio(MYSQL_THD thd, SYS_VAR *var,
                                                void *ptr, const void *val);
...
static MYSQL_SYSVAR_ENUM(
    ...
    SPEED_LIMIT_MODE_FIX_BANDWIDTH, &rpl_speed_limit_mode_typelib);

static MYSQL_SYSVAR_BOOL(
    enabled, rpl_speed_limit_enabled, PLUGIN_VAR_OPCMDARG,
...
static MYSQL_SYSVAR_ULONG(tick_interval, rpl_speed_limit_tick_interval,
                          PLUGIN_VAR_OPCMDARG, "Min sleep interval(ms)",
                          NULL,  // check
                          NULL,  // update
                          20, 5, 500, 1);
...
static MYSQL_SYSVAR_ULONG(slave_bandwidth, rpl_speed_limit_slave_bandwidth,
                          PLUGIN_VAR_OPCMDARG,
                          "target bandwidth to limit each slave(K/s)", NULL,
                          fix_rpl_speed_limit_slave_bandwidth, 10 * 1024UL,
                          1024UL, 1024UL * 1024UL,
                          1);  // def: 10M, min: 1M, max:1G

static MYSQL_SYSVAR_ULONG(total_bandwidth, rpl_speed_limit_total_bandwidth,
                          PLUGIN_VAR_OPCMDARG,
                          "total bandwidth shared by all slaves(K/s)", NULL,
                          fix_rpl_speed_limit_total_bandwidth, 50 * 1024UL,
                          10 * 1024UL, 4 * 1024UL * 1024UL,
                          1);  // def: 50M min: 10M, max: 4G
...
static MYSQL_SYSVAR_ULONG(run_test, rpl_speed_limit_run_test,
                          PLUGIN_VAR_OPCMDARG, "The time of running test.",
...
static void fix_rpl_speed_limit_mode(MYSQL_THD, SYS_VAR *, void *ptr,
  ...
  *static_cast<unsigned long *>(ptr) =
      *const_cast<unsigned long *>(static_cast<const unsigned long *>(val));
  speed_monitor.updateConf(true, false);
}

static void fix_rpl_speed_limit_enabled(MYSQL_THD, SYS_VAR *, void *ptr,
                                        const void *val) {
  *static_cast<char *>(ptr) =
      *const_cast<char *>(static_cast<const char *>(val));
...
static void fix_rpl_speed_limit_slave_bandwidth(MYSQL_THD, SYS_VAR *, void *ptr,
                                                const void *val) {
  *static_cast<unsigned long *>(ptr) =
      *const_cast<unsigned long *>(static_cast<const unsigned long *>(val));
...
static void fix_rpl_speed_limit_total_bandwidth(MYSQL_THD, SYS_VAR *, void *ptr,
                                                const void *val) {
  *static_cast<unsigned long *>(ptr) =
      *const_cast<unsigned long *>(static_cast<const unsigned long *>(val));
...
static void fix_rpl_speed_limit_max_token_ratio(MYSQL_THD, SYS_VAR *, void *ptr,
                                                const void *val) {
  *static_cast<unsigned long *>(ptr) =
      *const_cast<unsigned long *>(static_cast<const unsigned long *>(val));
  speed_monitor.updateConf(true, false);
...
static void fix_rpl_speed_limit_trace_level(MYSQL_THD, SYS_VAR *, void *ptr,
                                            const void *val) {
  *static_cast<unsigned long *>(ptr) =
      *const_cast<unsigned long *>(static_cast<const unsigned long *>(val));
...
static int check_rpl_speed_limit_run_test(MYSQL_THD, SYS_VAR *, void *save,
                                          struct st_mysql_value *value) {
