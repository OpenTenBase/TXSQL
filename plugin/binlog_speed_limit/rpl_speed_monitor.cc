...
extern PSI_mutex_key key_speed_monitor_mutex;
extern PSI_mutex_key key_speed_limit_mutex;
...
RplSpeedLimit::RplSpeedLimit(THD* thd)
  :  m_flags(0),
     m_thd(thd) {
  mysql_mutex_init(key_speed_limit_mutex,
...
void RplSpeedMonitor::init() {
  ...
  mysql_mutex_init(key_speed_monitor_mutex,
    &m_mutex, MY_MUTEX_INIT_FAST);

  pthread_key_create(&THD_RPL_SPEED_LIMIT, NULL);

  if (mysql_thread_create(PSI_NOT_INSTRUMENTED, &m_thread,
