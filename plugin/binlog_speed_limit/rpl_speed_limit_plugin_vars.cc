...
PSI_mutex_key key_speed_monitor_mutex;
PSI_mutex_key key_speed_limit_mutex;

static PSI_mutex_info all_speed_limit_mutexes[] = {
  { &key_speed_monitor_mutex, "Speed_monitor::m_mutex", 0, 0, ""},
  { &key_speed_limit_mutex ,  "Speed_limit::m_mutex",   0, 0, ""}
};

void init_psi_keys(void) {
  const char* category = "speed_limit";
  int count;

  count = static_cast<int>(array_elements(all_speed_limit_mutexes));
  mysql_mutex_register(category, all_speed_limit_mutexes, count);
}
...
static MYSQL_SYSVAR_ULONG(trace_level, rpl_speed_limit_trace_level,
  PLUGIN_VAR_OPCMDARG,
  "The tracing level for replication speed_limit.",
  NULL,               // check
...
SYS_VAR* rpl_speed_limit_system_vars[] = {
...
static void fix_rpl_speed_limit_mode(MYSQL_THD,
                                     SYS_VAR *,
                                     void *ptr,
                                     const void *val) {
  *(unsigned long*)ptr = *(static_cast<const unsigned long*>(val));
  speed_monitor.updateConf(true, false);
}

static void fix_rpl_speed_limit_enabled(MYSQL_THD,
                                        SYS_VAR *,
                                        void *ptr,
                                        const void *val) {
  *(char *)ptr = *(static_cast<const char *>(val));
...
static void fix_rpl_speed_limit_slave_bandwidth(MYSQL_THD,
                                                SYS_VAR *,
                                                void *ptr,
                                                const void *val) {
  *(unsigned long*)ptr = *(static_cast<const unsigned long*>(val));
...
static void fix_rpl_speed_limit_total_bandwidth(MYSQL_THD,
                                                SYS_VAR *,
                                                void *ptr,
                                                const void *val) {
  *(unsigned long*)ptr = *(static_cast<const unsigned long*>(val));
...
static void fix_rpl_speed_limit_max_token_ratio(MYSQL_THD,
                                                SYS_VAR *,
                                                void *ptr,
                                                const void *val) {
  *(unsigned long*)ptr = *(static_cast<const unsigned long*>(val));
...
static void fix_rpl_speed_limit_trace_level(MYSQL_THD,
                                            SYS_VAR *,
                                            void *ptr,
                                            const void *val) {
  *(unsigned long *)ptr = *(static_cast<const unsigned long *>(val));
...
static void fix_rpl_speed_limit_run_test(MYSQL_THD,
                                         SYS_VAR *,
                                         void *ptr,
                                         const void *val) {
  *(unsigned long *)ptr = *(static_cast<const unsigned long *>(val));
  if (rpl_speed_limit_run_test > 0) {
    ...
    rpl_speed_limit_run_test = 0;
  }

  return;
}

static int check_rpl_speed_limit_run_test(MYSQL_THD,
                                          SYS_VAR *,
                                          void *save,
                                          struct st_mysql_value *value) {
  /* Run test only if fix_bandwidth is enabled. */
  if (rpl_speed_limit_mode != SPEED_LIMIT_MODE_FIX_BANDWIDTH
