...
void load_or_flush_data(Callback_type type) {
  ...
  Firewall_callback_args *args = NULL;
  switch (type)
  {
    case USERMODE_LOAD:
      args = new Firewall_callback_args(cdb_firewall_manager.userhost_mode_cache, thd, type);
      break;
    case WHITELIST_LOAD:
    case WHITELIST_FLUSH:
      args = new Firewall_callback_args(cdb_firewall_manager.whitelist_cache, thd, type);
      break;
...
bool Cursor::store_field_values_and_flush(Field_idx_values_sequence &field_idx_and_values) {
  TABLE *table = m_table_list->table;
  Field **fields = table->field;

  empty_record(table);
  for (auto it = field_idx_and_values.begin(); it != field_idx_and_values.end(); it++) {
    if (fields[it->first]->store(it->second.c_str(),
                                 it->second.length(),
                                 &my_charset_bin))
      return true;
  }
