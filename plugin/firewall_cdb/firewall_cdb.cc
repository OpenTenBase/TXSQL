...
static void init_firewall_cdb_psi_keys() {
  PSI_rwlock_info all_firewall_cdb_rwlocks[] = {
      {&cdb_firewall_manager.whitelist_cache->key_rwlock_LOCK_whitelist_cache,
       "LOCK_plugin_firewallcdb_whitelist", 0, 0, PSI_DOCUMENT_ME},
      {&cdb_firewall_manager.userhost_mode_cache
            ->key_rwlock_LOCK_usermode_cache,
       "LOCK_plugin_firewallcdb_whitelist", 0, 0, PSI_DOCUMENT_ME}};
...
int CDB_Firewall_Manager::check_sql(THD *thd MY_ATTRIBUTE((unused))) {
  ...
  assert(ULLONG_MAX / 2 > access_granted);
  assert(ULLONG_MAX / 2 > access_denied);
  assert(ULLONG_MAX / 2 > access_suspicious);
  assert(ULLONG_MAX / 2 > cached_entries);

  /* firewall not enable or SUPER_ACL */
  if (cdb_firewall_manager.firewall_mode == false || check_access()) return 0;
  ...
  if (thd->query().length == 0) return 0;
  ...
  if (mode == Userhost_Mode_Cache::OFF || mode == Userhost_Mode_Cache::NONE)
    return 0;
  ...
  if (mode == Userhost_Mode_Cache::RECORDING) {
    /*
      ...
      If we call whitelist_cache->record_sql after
      whitelist_cache->query_in_whitelist, digest(normalized_query) may be
      caculated twice.
    */
    if (whitelist_cache->record_sql(thd, userhost)) cached_entries++;
    return 0;
  }
  ...
  */
  if (cdb_firewall_manager.firewall_trace ||
      mode == Userhost_Mode_Cache::DETECT) {
    std::string message;
  ...
  if (query_rejected) /* PROTECTING mode */
    access_denied++;
  else
    /* DETECTING mode */
    access_suspicious++;
...
static int firewall_cdb_plugin_init(void *arg MY_ATTRIBUTE((unused))) {
  ...
  if (cdb_firewall_manager.init()) return 1;
...
static MYSQL_SYSVAR_BOOL(
    ...
    cdb_firewall_manager.firewall_mode, PLUGIN_VAR_NOCMDARG,
    "Whether CDB Firewall is enabled or disabled (the default)", nullptr,
    nullptr, false);

static MYSQL_SYSVAR_BOOL(
    trace, /* firewall_cdb_trace*/
    cdb_firewall_manager.firewall_trace, PLUGIN_VAR_NOCMDARG,
...
static SYS_VAR *firewall_system_variables[] = {MYSQL_SYSVAR(mode),
                                               MYSQL_SYSVAR(trace), nullptr};
...
static SHOW_VAR firewall_system_status[] = {
    {"Firewall_access_denied",
     const_cast<char *>(reinterpret_cast<volatile char *>(
         &cdb_firewall_manager.access_denied)),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"Firewall_access_granted",
     const_cast<char *>(reinterpret_cast<volatile char *>(
         &cdb_firewall_manager.access_granted)),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"Firewall_access_suspicious",
     const_cast<char *>(reinterpret_cast<volatile char *>(
         &cdb_firewall_manager.access_suspicious)),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
    {"Firewall_cached_entries",
     const_cast<char *>(reinterpret_cast<volatile char *>(
         &cdb_firewall_manager.cached_entries)),
     SHOW_LONGLONG, SHOW_SCOPE_GLOBAL},
