...
bool parse(MYSQL_THD thd, const std::string &query, bool is_prepared = false) {
  MYSQL_LEX_STRING query_str = {const_cast<char *>(query.c_str()), query.length()};
...
void *normalize_callback(void *p_args) {
  normalize_callback_args *args = pointer_cast<normalize_callback_args *> (p_args);
...
std::string get_digest_hash(std::string digest) {
  ...
  SHA_EVP256(reinterpret_cast<const uchar*>(digest.c_str()), digest.length(), buf);
  std::string ret(reinterpret_cast<char*>(buf), sizeof(buf));
  return ret;
}


bool Whitelist_Cache::insert_rule(Userhost userhost, Digest digest, bool locked) {
  if (!locked)
    wrlock();
...
bool Whitelist_Cache::query_in_whitelist(THD *thd, Userhost &userhost) {
  ...
  */
  ...
  for (auto it_whitelist = it_range.first;
       it_whitelist!= it_range.second;
       it_whitelist++) {
...
bool Whitelist_Cache::record_sql(THD *thd, Userhost &userhost) {
  ...
  if (whitelist_it == whitelist_map->end())
    goto query_not_in_whitelist;
...
void Whitelist_Cache::do_load_data_from_table(THD *thd) {
  ...
  for (; whitelist_cursor.is_finished() == false; whitelist_cursor.read()) {
    Userhost userhost;
    Digest digest;
    whitelist_cursor.copy_and_set(&userhost, whitelist_cursor.userhost_column());
...
bool Whitelist_Cache::do_flush_to_disk(THD *thd) {
  ...
  for (auto it = m_tmp_whitelist.begin(); it != m_tmp_whitelist.end(); it++) {
    ...
    field_idx_and_values.emplace_back(whitelist_cursor.userhost_column(), userhost);
...
bool Whitelist_Cache::fill_i_s_table(THD *thd, TABLE_LIST *tables) {
  ...
  rdlock();
  for (auto it_w = m_whitelist_map.begin(); it_w != m_whitelist_map.end(); it_w++) {
    ...
    for (auto it_u = user_map.begin(); it_u != user_map.end(); it_u ++) {
      ...
      table->field[0]->store(userhost.c_str(), userhost.length(), system_charset_info);
      table->field[1]->store(digest.c_str(), digest.length(), system_charset_info);
