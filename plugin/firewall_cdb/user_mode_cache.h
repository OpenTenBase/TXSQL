...
class Userhost_Mode_Cache {
 public:
  ...
  ~Userhost_Mode_Cache() { mysql_rwlock_destroy(&LOCK_usermode_cache); }

  enum Userhost_Mode { OFF, PROTECT, DETECT, RECORDING, NONE };
 ...
 private:
  ...
  Userhost_Mode string_to_mode(std::string str_mode) {
    if (str_mode == "OFF") return OFF;
    if (str_mode == "PROTECT") return PROTECT;
    if (str_mode == "DETECT") return DETECT;
    if (str_mode == "RECORDING") return RECORDING;
    return NONE;
  }

  std::string *mode_to_string(Userhost_Mode mode) {
    static std::string mode_str[] = {"OFF", "PROTECT", "DETECT", "RECORDING",
                                     "NONE"};
    return &mode_str[(uint)mode];
  }

  typedef malloc_unordered_map<std::string, Userhost_Mode> Userhost_Mode_Map;
  typedef malloc_unordered_map<std::string, Userhost_Mode>::iterator
      Userhost_Mode_Map_Iterator;
