...
PSI_mutex_key  key_LOCK_kms;
}  // namespace keyring_kms

extern mysql_rwlock_t LOCK_keyring;
extern mysql_mutex_t  LOCK_kms;
...
static PSI_rwlock_info all_keyring_rwlocks[] = {
    {&keyring_kms::key_LOCK_keyring, "LOCK_keyring", 0, 0, PSI_DOCUMENT_ME}};

static PSI_mutex_info all_keyring_mutexes[] = {
    {&keyring_kms::key_LOCK_kms, "LOCK_kms", 0, 0, PSI_DOCUMENT_ME}};

static PSI_memory_info all_keyring_memory[] = {
    {&keyring_kms::key_memory_KEYRING, "KEYRING", 0, 0, PSI_DOCUMENT_ME}};
...
void keyring_init_psi_keys(void) {
  ...
  count= static_cast<int>(array_elements(all_keyring_mutexes));
  mysql_mutex_register(category, all_keyring_mutexes, count);
}
#endif  // HAVE_PSI_INTERFACE

bool init_keyring_locks() {
  if (mysql_rwlock_init(keyring_kms::key_LOCK_keyring, &LOCK_keyring))
    return true;

  if (mysql_mutex_init(keyring_kms::key_LOCK_kms, &LOCK_kms, MY_MUTEX_INIT_FAST))
    return true;
...
bool mysql_key_fetch(std::unique_ptr<IKey> key_to_fetch, char **key_type,
  ...
  if (fetched_key) {
    ...
    *key_type = my_strdup(keyring_kms::key_memory_KEYRING,
...
bool mysql_keyring_iterator_get_key(Keys_iterator *key_iterator, char *key_id,
                                    char *user_id) {
  keyring_kms::Key_metadata *key_loaded = NULL;
