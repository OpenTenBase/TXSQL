    ...
    size_t *outlen, const tcsm_sms4_key_t *key, int enc,int no_padding)
{
  if (enc) {
    ...
    size_t i = 0;
    for (; i < len_after_padding/SMS4_BLOCK_SIZE; ++i) {
      tcsm_sms4_encrypt(buf+i*SMS4_BLOCK_SIZE, out+i*SMS4_BLOCK_SIZE, key);
    }
    *outlen = len_after_padding;
    ...
    size_t i = 0;
    for (; i < inlen/SMS4_BLOCK_SIZE; ++i) {
      tcsm_sms4_decrypt(in+i*SMS4_BLOCK_SIZE, out+i*SMS4_BLOCK_SIZE, key);
    }
