...
public class SMKeyManager {
    ...
    public static SMKeyManager getInstance(String keypath) {
        if(mInstance == null) {
            synchronized (SMKeyManager.class) {
                if(mInstance == null) {
                    mInstance = new SMKeyManager(keypath);
                }
            }
        }
        return mInstance;
     ...
     */
    private SMKeyManager(String keypath) {
        mSMKeyManUtils = SMKeyManUtils.getInstance();
        if(TextUtils.isEmpty(keypath)) {
            mKeyPath = new File("./").getAbsolutePath();
        } else {
            mKeyPath = keypath;
        }
    }
