...
public class ExportSM2KeyActivity extends BaseToolbarActivity implements View.OnClickListener{
    ...
    public static Intent getStartIntent(Context context, String backtxt) {
        Intent intent = new Intent(context, ExportSM2KeyActivity.class);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setTitle((String)getText(R.string.export_keypair));
    ...
    private void setContent(boolean init, String desc) {
        if(init) {
            mDataItems[1] = mDataItems[3] = "empty";
        } else {
            mDataItems[1] = mDataItems[3] = "export failed!";
        }
        if(keymanager != null && desc != null) {
            String[] keyPair = keymanager.getAsymPriKeypair(desc);
            if(keyPair != null) {
                mDataItems[1] = keyPair[1];
                mDataItems[3] = keyPair[0];
            }
        }
        mAdapter.clear();
