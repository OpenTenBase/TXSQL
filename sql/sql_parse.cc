...
void init_sql_command_flags() {
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_STATUS_FUNC] =
      CF_STATUS_COMMAND | CF_REEXECUTION_FRAGILE | CF_HAS_RESULT_SET;
  sql_command_flags[SQLCOM_SHOW_TLOGS] = CF_STATUS_COMMAND;
  ...
  */
  ...
  sql_command_flags[SQLCOM_CREATE_RESOURCE_GROUP] =
      ...
      CF_ALLOW_ACCESS_RESOURCE_GROUP_EXTRA_SCHEMA;
  ...
  */
  ...
  sql_command_flags[SQLCOM_SHOW_TLOGS] |= CF_ALLOW_PROTOCOL_PLUGIN;
...
static inline void binlog_gtid_end_transaction(THD *thd) {
  ...
  */
  if (thd->lex->sql_command == SQLCOM_COMMIT ||
      ...
      // if the xa rollback is injected by coordinate ,should not call
      // gtid_end_transaction to  insert trx into binlog(gtid)
      ((thd->lex->sql_command == SQLCOM_XA_ROLLBACK) &&
       ((!thd->rli_slave) || (!thd->rollback_injected_by_coord))) ||
