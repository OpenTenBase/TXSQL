...
class MYSQL_BIN_LOG : public TC_LOG {
 ...
 private:
  ...
  /** The instrumentation key to use for @ LOCK_clear_flush_trxs. */
  PSI_mutex_key m_key_LOCK_clear_flush_trxs;
  /** The instrumentation key to use for @ update_cond. */
  PSI_cond_key m_key_update_cond;
  /** The instrumentation key to use for @ prep_xids_cond. */
  PSI_cond_key m_key_prep_xids_cond;
  /** The instrumentation key to use for @ clear_flush_trxs. */
  PSI_cond_key m_key_clear_flush_trxs_cond;
  ...
  */
  ...
  void set_psi_keys(
      ...
      PSI_mutex_key key_LOCK_clear_flush_trxs,
      PSI_cond_key key_clear_flush_trxs_cond) {
    ...
    m_key_LOCK_clear_flush_trxs = key_LOCK_clear_flush_trxs;
    m_key_clear_flush_trxs_cond = key_clear_flush_trxs_cond;
  }
 ...
 private:
  mysql_mutex_t LOCK_clear_flush_trxs;
  mysql_cond_t m_clear_flush_trxs_cond;
  bool m_waiting_flush_trxs_finish{false};
  // use a non atomic counter to record the flush trxs, must read it after
  // hold the LOCK_log without new flush trxs
  ulonglong flush_trxs{0};
  std::atomic<ulonglong> m_atomic_commit_trxs{0};
  void trx_signal_commit();
