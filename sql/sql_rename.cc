...
static void create_recycle_bin_drop_log(THD *thd, TABLE_LIST *table_list,
                                        String &drop_log) {
  drop_log.append("RENAME TABLE ");
  for (TABLE_LIST *table = table_list; table && table->next_local;
    ...
    append_table_ident(thd, &drop_log, old_table, false);
    drop_log.append(" TO ");
    append_table_ident(thd, &drop_log, new_table, true);
    drop_log.append(",");
  }
  drop_log.chop();
  ...
  for (TABLE_LIST *table = table_list; table;
    ...
    if (op == RB_RECYCLE_TABLE_BY_DROP || op == RB_RECYCLE_TABLE_BY_RENAME ||
      ...
      struct my_timeval tp = {
          tv.m_tv_sec + static_cast<int64>(cdb_recycle_bin_retention),
          tv.m_tv_usec};
      record.set_purge_time(tp);

      info.push_back(std::move(record));
    } else if (op == RB_RECOVERY_TABLE) {
      if (old_table->get_table_name()) {
...
bool mysql_rename_tables(THD *thd, TABLE_LIST *table_list) {
  ...
  /**
    ...
    recycle bin
  */
  const bool is_recycle_bin_enabled = recycle_bin_enabled(thd);
  ...
  */
  ...
  for (TABLE_LIST *table = table_list; table && table->next_local;
    ...
    */
    if (is_recycle_bin_enabled && is_ren_table && table_def &&
      ...
      if (my_strcasecmp(table_alias_charset, table->db,
        ...
        for (const dd::Foreign_key *fk : table_def->foreign_keys()) {
          ...
          if (thd->lex->recycle_bin_op == RB_RECOVERY_TABLE)
  ...
  */
  if (is_recycle_bin_enabled) {
    if (!error && (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
                   thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE ||
                   thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_RENAME ||
                   thd->lex->recycle_bin_op == RB_RECOVERY_TABLE)) {
      ...
      */
      ...
      } else if (rb_op == RB_RECOVERY_TABLE) {
  ...
  bool is_truncate_table = is_recycle_bin_enabled &&
    thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE;
