...
/**
  create binlog log log of recycle table by drop table,
  convert drop table to rename table binlog.

  @param[in]      table_list       TABLE_LIST for the identifier

  @return         the binlog for drop table of recycle bin
*/

static void create_recycle_bin_log(THD *thd, TABLE_LIST *table_list,
                                   String &recycle_bin_log) {
  if (!table_list) return;

  recycle_bin_log.append("RENAME TABLE ");
  for (TABLE_LIST *table = table_list; table && table->next_local;
       table = table->next_local->next_local) {
    TABLE_LIST *old_table = table;
    assert(old_table->get_table_name());
    TABLE_LIST *new_table = table->next_local;
    assert(new_table->get_table_name());
    append_table_ident(thd, &recycle_bin_log, old_table, false);
    recycle_bin_log.append(" TO ");
    append_table_ident(thd, &recycle_bin_log, new_table, true);
    recycle_bin_log.append(",");
  }
  recycle_bin_log.chop();
  recycle_bin_log.append(" /* generated by cdb recycle bin in server */");
}

static bool rebuild_table_list_for_drop_if_exists(THD *thd,
                                                  TABLE_LIST *table_list,
                                                  TABLE_LIST **rename_list,
                                                  TABLE_LIST **drop_list) {
  TABLE_LIST *table = nullptr, *to_table = nullptr, *prev = nullptr,
             *rename_list_head = nullptr, *drop_list_head = nullptr;
  const dd::Table *table_def = nullptr;
  std::vector<TABLE_LIST *> not_exists;
  for (table = table_list; table; table = to_table->next_local) {
    to_table = table->next_local;
    if (thd->dd_client()->acquire(table->db, table->table_name, &table_def)) {
      return true;
    }
    if (!table_def) {
      not_exists.push_back(table);
    } else {
      if (prev == nullptr) {
        rename_list_head = table;
      } else {
         prev->next_local = table;
         prev->next_global = table;
      }
      prev = to_table;
    }
  }
  if (prev) {
    prev->next_global = nullptr;
    prev->next_local = nullptr;
  }
  
  prev = nullptr;
  for (auto drop_table : not_exists) {
    to_table = drop_table->next_local;
    if (prev == nullptr) {
      drop_list_head = drop_table;
    } else {
      prev->next_global = drop_table;
      prev->next_local = drop_table;
    }
    prev = to_table;
  }
  if (prev) {
  ...
  *rename_list = rename_list_head;
  *drop_list = drop_list_head;
  return false;
}

static void build_drop_log(THD *thd, TABLE_LIST *table_list,
                           String &drop_log) {
  if (!table_list) return;

  drop_log.append("DROP TABLE IF EXISTS ");
  for (TABLE_LIST *table = table_list; table && table->next_local;
       table = table->next_local->next_local) {
    assert(table->get_table_name());
    append_table_ident(thd, &drop_log, table, false);
    drop_log.append(",");
  }
  drop_log.chop();
  drop_log.append(" /* generated by cdb recycle bin in server */");
}

/**
  Insert records in recycle_bin_info when creating the recycle
  table and delete records in the recycle bin info when the
  recovery table is created.

   @param[in]      op               Thread handler
   @param[in]      thd              Identifier
   @param[in]      table_list       TABLE_LIST for the identifier
   @param[out]     info             Current parsing context, if available
*/

static void create_recycle_bin_table_info(
    enum_recycle_bin_op op, THD *thd, TABLE_LIST *table_list,
    std::vector<Recycle_table_record> &info) {
  for (TABLE_LIST *table = table_list; table;
       table = table->next_local->next_local) {
    TABLE_LIST *old_table = table;
    TABLE_LIST *new_table = table->next_local;
    Recycle_table_record record;
    if (op == RB_RECYCLE_TABLE_BY_DROP || op == RB_RECYCLE_TABLE_BY_RENAME ||
        op == RB_RECYCLE_TABLE_BY_TRUNCATE ||
        op == RB_RECYCLE_TABLE_BY_DROP_DATABASE) {
      if (new_table->get_table_name())
        record.set_table_name(new_table->get_table_name());

      if (old_table->get_db_name())
        record.set_origin_schema(old_table->get_db_name());

      if (old_table->get_table_name())
        record.set_origin_table(old_table->get_table_name());

      struct my_timeval tv = thd->query_start_timeval_trunc(2);
      record.set_drop_time(tv);

      struct my_timeval tp = {
          tv.m_tv_sec + static_cast<int64>(txsql_recycle_bin_retention),
          tv.m_tv_usec};
      record.set_purge_time(tp);

      info.push_back(std::move(record));
    } else if (op == RB_RECOVERY_TABLE_BY_RENAME ||
               op == RB_RECOVERY_TABLE_BY_RESTORE) {
      if (old_table->get_table_name()) {
        record.set_table_name(old_table->get_table_name());
        info.push_back(std::move(record));
      }
    } else {
      assert(false);
...
bool mysql_rename_tables(THD *thd, TABLE_LIST *table_list) {
  ...
  /**
    The drop table query log is convert rename table query log on opening
    recycle bin
  */
  char recycle_bin_log_buf[256];
  String recycle_bin_log(recycle_bin_log_buf, sizeof(recycle_bin_log_buf),
                         system_charset_info);
  recycle_bin_log.length(0);
  char drop_log_buf[256];
  String drop_log(drop_log_buf, sizeof(drop_log_buf), system_charset_info);
  drop_log.length(0);
  ...
  */
  ...
  /* The tables and triggers were locked when drop database. */
  if (thd->lex->recycle_bin_op != RB_RECYCLE_TABLE_BY_DROP_DATABASE &&
  ...
  if (thd->lex->drop_if_exists &&
    ...
    TABLE_LIST *rename_table_list, *drop_table_list;
    bool error = rebuild_table_list_for_drop_if_exists(
        thd, table_list, &rename_table_list, &drop_table_list);
    if (error) {
      my_error(ER_RECYCLE_BIN_ERROR_OCCURRED, MYF(0),
               "rebuild table list for drop if exiests failed!");
      return true;
    }
    table_list = rename_table_list;
    build_drop_log(thd, drop_table_list, drop_log);
  }

  /**
    The elements in the vector record the information related to each table 
    in the rename table list: the name of the table placed in the recycle bin, 
    the name of the table and the db name before recycling, and the time when 
    it was dropped and the time when it was purged.
  */
  std::vector<Recycle_table_record> recycle_table_info;
  ...
  if (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
    ...
    create_recycle_bin_log(thd, table_list, recycle_bin_log);
  }

  if (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
      thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE ||
      thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_RENAME ||
      thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RENAME ||
      thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RESTORE ||
      thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP_DATABASE) {
    create_recycle_bin_table_info(thd->lex->recycle_bin_op, thd, table_list,
                                  recycle_table_info);
  }

  const dd::Table *table_def = nullptr;
  bool is_ren_table = false;
  for (TABLE_LIST *table = table_list; table && table->next_local;
    ...
    /*
      Foregin key check: not allow a table as parent of other table.
    */
    if (is_ren_table && table_def &&
      ...
      if (!is_recycle_bin_db(table->db, table->db_length)) {
        for (const dd::Foreign_key_parent *fk :
          ...
          if (my_strcasecmp(table_alias_charset,
                            fk->child_schema_name().c_str(), table->db) == 0 &&
          ...
          if (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
            ...
            my_error(ER_TRUNCATE_ILLEGAL_FK, MYF(0), fk_info_str(thd, fk));
          else
            assert(0);
        ...
        for (const dd::Foreign_key *fk : table_def->foreign_keys()) {
          ...
          if (thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RENAME ||
              thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RESTORE)
            my_error(ER_CDB_RECYCLE_BIN_ERROR_IN_RECOVERY_TABLE, MYF(0),
      ...
      /**
        Note that: Foreign key is not allowed in truncate table, because the
        create table like statement does not preserve any foreign key
        definitions.
      */
      if (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE &&
          table_def->foreign_keys().size() > 0) {
        int err = ER_CDB_RECYCLE_BIN_ERROR_IN_CHECK_SOURCE_TABLE;
        my_error(err, MYF(0), "\"truncate table with foreign key\"");
        return true;
      }
    }
  }
  ...
  */
  if (!error &&
      (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
       ...
       thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RESTORE ||
       thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP_DATABASE)) {
    enum enum_recycle_bin_op rb_op = thd->lex->recycle_bin_op;
    /*
      switch context for update data dictionary, it include not write binlog,
      switch lex context etc.
    */
    Recycle_bin_persistor persistor;
    if (rb_op == RB_RECYCLE_TABLE_BY_DROP ||
        rb_op == RB_RECYCLE_TABLE_BY_TRUNCATE ||
        rb_op == RB_RECYCLE_TABLE_BY_RENAME ||
        rb_op == RB_RECYCLE_TABLE_BY_DROP_DATABASE) {
      error = persistor.save(current_thd, recycle_table_info);
    } else if (rb_op == RB_RECOVERY_TABLE_BY_RENAME ||
               rb_op == RB_RECOVERY_TABLE_BY_RESTORE) {
      error = persistor.drop(current_thd, recycle_table_info);
    } else {
      assert(false);
    }
  }

  bool is_trans_op = false;
  char query_buf[256];
  String create_like_query(query_buf, sizeof(query_buf), system_charset_info);
  create_like_query.length(0);
  handlerton *create_post_ddl_ht = nullptr;
  TABLE_LIST *like_table = nullptr;
  bool is_truncate_table =
      thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE;
  HA_CREATE_INFO create_info;
  if (is_truncate_table && !error) {
    /**
      Create table <truncated_table> like <recycled_table>
      ====================================================
      First, because "TRUNCATE TABLE Statement" support only one table, so we
      just process one table situation.

      The processes step is:
      1. Build the "create table like" statement binlog;
      2. Construct the environment about mysql_create_like_table
      3. execute mysql_create_like_table
      4. write rename table binlog log
      5. commit trans and write create table like query log
    */

    /**
      step1. build query log
    */
    /*
      Not support temporary table: If the truncated table is temporary table,
      a error will occur in rename phase.
    */
    LEX *saved_lex = thd->lex;
    thd->lex = new (thd->mem_root) st_lex_local;
    lex_start(thd);
    thd->lex->recycle_bin_op = saved_lex->recycle_bin_op;
    create_like_query.append("CREATE TABLE ");
    TABLE_LIST *create_table = table_list;
    assert(create_table);
    append_table_ident(thd, &create_like_query, create_table, false);
    create_like_query.append(" LIKE ");
    like_table = table_list->next_local;
    assert(like_table);
    append_identifier(thd, &create_like_query, RECYCLE_BIN_SCHEMA_NAME.str,
                      RECYCLE_BIN_SCHEMA_NAME.length, system_charset_info,
                      thd->charset());
    create_like_query.append('.');
    append_identifier(thd, &create_like_query, like_table->table_name,
                      like_table->table_name_length, system_charset_info,
                      thd->charset());
    create_like_query.append(" /* generated by cdb recycle bin in server */");

    /**
      step2. build create_info structure
    */
    // from PT_create_table_stmt::make_cmd
    create_table->open_strategy = TABLE_LIST::OPEN_FOR_CREATE;
    create_table->open_type = OT_BASE_ONLY;
    create_info.default_table_charset = nullptr;
    create_info.options |= HA_LEX_CREATE_TABLE_LIKE;

    create_info.options |= HA_LEX_CREATE_TABLE_LIKE;
    /**
       @todo: temporary table is forbidden
    */
    assert(!(create_info.options & HA_LEX_CREATE_TMP_TABLE));
    assert(thd->lex->create_info == nullptr);
    thd->lex->create_info = &create_info;
    thd->lex->sql_command = SQLCOM_CREATE_TABLE;
    assert(create_table->next_global == like_table);
    thd->lex->query_tables = create_table;

    // From Sql_cmd_create_table::execute
    ...
    /**
      step3. execute mysql_create_like_table
        1. do not write binlog
        2. do not commit trans
        3. do not post ddl log
        4. do not lock table and release lock
    */
    Strict_error_handler strict_handler;
    /* Push Strict_error_handler */
    if (!thd->lex->is_ignore() && thd->is_strict_mode())
      thd->push_internal_handler(&strict_handler);
    error = mysql_create_like_table(thd, create_table, like_table, &create_info,
                                    &create_post_ddl_ht, &is_trans_op);
    if (!thd->lex->is_ignore() && thd->is_strict_mode())
      thd->pop_internal_handler();
    thd->lex->cleanup(thd, true);
    thd->lex->destroy();
    lex_end(thd->lex);
    delete static_cast<st_lex_local *>(thd->lex);
    thd->lex = saved_lex;
    /**
      Note that: if create table like fail, we must be invalidate the table
      definition. Otherwise, it will occur the create table and the like table
      use the same table share.
    */
    if (error) like_table->table->invalidate_dict();
    if (error && int_commit_done) {
      // Rollback rename table if these tables have no-automatic ddl
      table_list = reverse_table_list(table_list);
      int_commit_full_revert = !rename_tables(thd, table_list, &int_commit_done,
                                              &post_ddl_htons, &fk_invalidator);
  ...
  if (!error) {
    if (thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_DROP ||
         ...
         thd->lex->recycle_bin_op == RB_RECOVERY_TABLE_BY_RESTORE) {
      /**
        step4. write rename table log and commit transaction below
      */
      if (recycle_bin_log.length())
        error = write_bin_log(thd, true, recycle_bin_log.ptr(),
                              recycle_bin_log.length(), !int_commit_done);

      if (!error && thd->lex->recycle_bin_op == RB_RECYCLE_TABLE_BY_TRUNCATE) {
        /**
          step5. write create table like <recycle_table> query log and commit
                we ignore any error in this process
        */
       DBUG_EXECUTE_IF("rb_crash_before_commit1", DBUG_SUICIDE(););
       ...
       DBUG_EXECUTE_IF("rb_crash_after_commit1", DBUG_SUICIDE(););

        if (!error) {
          thd->get_transaction()->xid_state()->reset();
          thd->get_transaction()->xid_state()->set_query_id(next_query_id());
          error = write_bin_log(thd, true, create_like_query.ptr(),
                                create_like_query.length(), is_trans_op);
          assert(!error);
        }
      } else if (thd->lex->drop_if_exists) {
        DBUG_EXECUTE_IF("rb_crash_before_commit1", DBUG_SUICIDE(););
        thd->is_commit_in_middle_of_statement = true;
        error = (trans_commit_stmt(thd) || trans_commit_implicit(thd));
        thd->is_commit_in_middle_of_statement = false;
        ...
        if (!error) {
          ...
          thd->get_transaction()->xid_state()->set_query_id(next_query_id());
          if (drop_log.length())
            error = write_bin_log(thd, true, drop_log.ptr(), drop_log.length(),
                                  false);
          assert(!error);
        }
      }
  ...
  if (!error && (!int_commit_done || is_trans_op)) {
    error = (trans_commit_stmt(thd) || trans_commit_implicit(thd));
    DBUG_EXECUTE_IF("rb_crash_after_commit2", DBUG_SUICIDE(););

    if (is_truncate_table) assert(!error);
  ...
  if (create_post_ddl_ht) create_post_ddl_ht->post_ddl(thd);
  ...
  if (!error && thd->lex->recycle_bin_op != RB_RECYCLE_TABLE_BY_DROP_DATABASE)
    my_ok(thd);
