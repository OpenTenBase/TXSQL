...
void Error_context::report_node(const Json_dom *dom, Message err_code) {
  assert(!(err_code == Message::JSON_INVALID_TOTAL_FREQUENCY ||
...
/**
  ...
  It uses Field::store() or some variant to test if the value is in the field
...
Histogram *Histogram::json_to_histogram(MEM_ROOT *mem_root,
  ...
  if (histogram_type_dom->json_type() != enum_json_type::J_STRING) {
    context->report_node(histogram_type_dom, Message::JSON_WRONG_ATTRIBUTE_TYPE);
    return nullptr;
  }
  ...
  if (
      ...
      !context->internal() &&
#endif
      histogram->get_num_buckets_specified() < histogram->get_num_buckets()) {
  ...
  if (json_dom->json_type() == enum_json_type::J_OPAQUE) {
    ...
    */
    ...
    /*
      ...
      goes to the outside as standard json.

      So when the outside data comes back and is processed by parse_json(), it
      is J_STRING and needs to be decoded here. The fact that it is always
      prefixed "base64:type254:" could be also explored.
    */
    const Json_string *json_string = down_cast<const Json_string *>(json_dom);
    const std::string &str = json_string->value();
    ...
    const char *substr = str.c_str() + prefix_length;
    const size_t slen = str.size() - prefix_length;
    if (slen > base64_decode_max_arg_length()) return true;
    const size_t needed =
        static_cast<size_t>(base64_needed_decoded_length(slen));
    ...
    const int64 rlen =
        base64_decode(substr, slen, &base64_buffer[0], &end_ptr, 0);
    if (rlen < 0 ||
        (end_ptr && (static_cast<size_t>(end_ptr - substr) != slen))) {
      context->report_node(json_dom, Message::JSON_VALUE_FORMAT_ERROR);
      return true;
    }
    base64_buffer.length(rlen);
  ...
  if (json_dom->json_type() == enum_json_type::J_DECIMAL)
    *out = *down_cast<const Json_decimal *>(json_dom)->value();
    ...
    if (json_dom->json_type() == enum_json_type::J_INT) {
      ...
      if (ulonglong2decimal(
              down_cast<const Json_uint *>(json_dom)->value(), out)) {
        context->report_node(json_dom, Message::JSON_VALUE_FORMAT_ERROR);
        return true;
      }
...
bool update_histogram(THD *thd, TABLE_LIST *table, const columns_set &columns,
  ...
  */
  ...
  if (data.str != nullptr) {
    ...
    /*
      The column needs to be in the write set because Field::store() or some
      variant is used to test value domain.
    */
    bitmap_set_bit(tbl->write_set, field->field_index());

    // Parse the literal for a standard JSON object.
    String str{data.str, static_cast<uint32>(data.length),
               &my_charset_utf8mb4_bin};
    Json_dom_ptr dom;
    {
    ...
    /*
      pending histogram build as remote statistic task
    */
    if (auto_stats_node_selection != LOCAL &&
        Statistics_manager::is_running()) {
      my_time_t time_now = (my_time_t)thd->query_start_in_secs();
      bool create_task_ret =
          Statistics_manager::add_columns_to_task(table->db, table->db_length,
                                  table->table_name, table->table_name_length,
                                  resolved_fields, num_buckets, time_now);
      for (const Field *field : resolved_fields) {
        std::string col_name(field->field_name);
        if (create_task_ret) {
          results.emplace(col_name, Message::HISTOGRAM_AUTO_STATS_CREATE_FAILURE);
        } else {
          results.emplace(col_name, Message::HISTOGRAM_AUTO_STATS_CREATED);
        }
      }
      goto end;
    } else {
...
bool drop_histograms(THD *thd, TABLE_LIST &table, const columns_set &columns,
  ...
  if (Statistics_manager::is_running()) {
    Statistics_manager::drop(table.db, table.db_length,
                             table.table_name, table.table_name_length,
                             false, columns);
  }
  ...
  for (const std::string &column_name : columns) {
    ...
    if (Statistics_manager::is_running()) {
      // drop history histograms created by auto statistics
      // Note that column_statistics is null before apply histogram.
      Histogram_error_handler error_handler(thd);
    ...
    // need insure the init of history histogram table
    // to skip the error of upgrade**.test
    if (histogram_history_enabled && !Statistics_manager::is_running()) {
      // Dropping history histograms regardless of whether
      // histogram_history_enabled is open or not.
      Histogram_error_handler error_handler(thd);
      History_table_persistor persistor{nullptr};
      persistor.drop({table.db, table.db_length},
                     {table.table_name, table.table_name_length},
...
static bool rename_histogram(THD *thd, const char *old_schema_name,
  ...
  if (Statistics_manager::is_running()) {
    columns_set columns;
    Statistics_manager::drop(old_schema_name, strlen(old_schema_name),
                             old_table_name, strlen(old_table_name),
                             true /* is_rename */, columns);

    // direct rename history histogram even null column_statistics
  ...
  // need insure the init of history histogram table
  // to skip the error of upgrade**.test (e.g., rpl.rpl_mysql_upgrade)
  if (histogram_history_enabled && !Statistics_manager::is_running()) {
    // rename history histogram regardless of whether
    // histogram_history_enabled is open or not.
    Histogram_error_handler error_handler(thd);
    History_table_persistor persistor{nullptr};
    persistor.rename(old_schema_name, old_table_name, new_schema_name,
