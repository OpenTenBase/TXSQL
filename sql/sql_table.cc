...
void mysql_reset_mdl_request_for_try(THD *thd)
{
  ...
  for(table= table_begin; table && table != table_end; table= table->next_global)
  {
                                          ...
                                          bool &converted, handlerton *&db_type)
{
  if (converted)
  {
    ...
    converted= false;
    sql_print_information("%s db:[%s],tb[%s]"
           ...
           db_type->db_type == DB_TYPE_MYISAM &&
           strcasecmp(db, "mysql"))
  {
    ulong tmp_myisam_conversion_innodb;
    String print_prefix;
    tmp_myisam_conversion_innodb= txsql_convert_myisam_to_innodb;
    print_prefix.append("myisam_conversion_innodb[");
    print_prefix.append(txsql_convert_myisam_to_innodb_names[tmp_myisam_conversion_innodb]);
    print_prefix.append("]");
...
bool mysql_alter_table(THD *thd, const char *new_db, const char *new_name,
  ...
  */
  ...
  if (check_engine(thd, alter_ctx.new_db, alter_ctx.new_name, create_info, alter_info))
    return true;
