  ...
  for (size_t i = 0; i < td->size(); i++) {
    ...
    */
    ...
    if(!no_fill_output){
      ...
      size = log_event_print_value(&tmp_cache, is_null ? nullptr : value, td->type(i),
                                   td->field_metadata(i), typestr,
                                   sizeof(typestr), col_name, is_partial);
...
*/
bool Log_event::contains_partition_info(bool end_group_sets_max_dbs, bool table_mode) {
  bool res;
