...
bool copy_event_cache_to_string_and_reinit(IO_CACHE *cache, LEX_STRING *to) {
  ...
  if (cache->end_of_file > SIZE_T_MAX ||
      !(to->str = (char *)my_malloc(key_memory_log_event,
                                    (to->length = (size_t)cache->end_of_file),
                                    MYF(0)))) {
    perror(
        "Out of memory: can't allocate memory in "
        "copy_event_cache_to_string_and_reinit().");
    goto err;
  }
...
const char *Log_event::get_type_str() const {
  if (this->is_flashback) {
    switch (get_type_code()) {
      case binary_log::WRITE_ROWS_EVENT:
        return get_type_str(binary_log::DELETE_ROWS_EVENT);
      case binary_log::WRITE_ROWS_EVENT_V1:
        return get_type_str(binary_log::DELETE_ROWS_EVENT_V1);
      case binary_log::DELETE_ROWS_EVENT:
        return get_type_str(binary_log::WRITE_ROWS_EVENT);
      case binary_log::DELETE_ROWS_EVENT_V1:
        return get_type_str(binary_log::WRITE_ROWS_EVENT_V1);
      default:
        break;
    }
  }
...
/**
  ...
  @param[in] no_fill_output    true: just get row size, no output to file(used by flashback);
                               false: output data to file
  ...
  */
  ...
  for (size_t i = 0; i < td->size(); i++) {
    ...
    */
    ...
    if (!no_fill_output) {
      size = log_event_print_value(file, is_null ? nullptr : value, td->type(i),
                                   td->field_metadata(i), typestr,
...
void Rows_log_event::change_to_flashback_event(
  ...
  uchar *swap_buff1 = nullptr, *swap_buff2 = nullptr;
  ...
  */
  ...
  if (cmd_can_generate_row_events) {
    ...
    */
    ...
    assert(is_rb_truncate_or_drop_db ||
           trn_ctx->is_active(Transaction_ctx::SESSION));
    /* and the transaction's xid has been already computed. */
    assert(is_rb_truncate_or_drop_db ||
           !trn_ctx->xid_state()->get_xid()->is_null());
    ...
    */
    assert(is_rb_truncate_or_drop_db || thd->debug_binlog_xid_last.is_null() ||
...
//  DBUG_ASSERT(thd->owned_gtid.sidno > 0);
  ...
  assert(!thd->rollback_injected_by_coord);
