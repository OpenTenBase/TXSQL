...
bool Gtid_state::is_executed(const Gtid &gtid) const {
  ...
  /* 
    We also need to ensure that the GTID does not exist in Owned_gtids, 
    or exist both in Owned_gtids and Undeleted_gtids.
  */
  if (cdb_optimize_gtid_lock) {
    sid_locks_undeleted.assert_not_owner(gtid.sidno);
    gtid_state->lock_sidno_undeleted(gtid.sidno);
    if (ret) {
      if (owned_gtids.contains_gtid(gtid) && !undeleted_gtids.contains_gtid(gtid)) {
        /* GTID exists in executed_gtids and owned_gtids but not in undeleted_gtids */
        ret = false;
      }
    }
    gtid_state->unlock_sidno_undeleted(gtid.sidno);
  }
  return ret;
}
...
void Gtid_state::add_executed_gtid(const Gtid &gtid, rpl_sidno *locked_sidno) {
  DBUG_TRACE;
  global_sid_lock->assert_some_lock();
  ...
  */
  bool need_to_lock_sidno =
      (locked_sidno_was_passed_null || *locked_sidno != gtid.sidno);
  ...
  if (need_to_lock_sidno) {
    ...
    */
    ...
    if (!locked_sidno_was_passed_null) *locked_sidno = gtid.sidno;
  }
  executed_gtids._add_gtid(gtid);

  /* The caller will unlock the sidno_lock if locked_sidno was passed */
  if (locked_sidno_was_passed_null) unlock_sidno(gtid.sidno);
}
    ...
    for (int sidno = 1; sidno <= todo.get_max_sidno(); sidno++) {
      ...
      while (todo.contains_sidno(sidno)) {
        ...
        if (cdb_optimize_gtid_lock) {
          ...
          Gtid_set actual_executed_gtids(&sid_map, nullptr);
...
enum_return_status Gtid_state::ensure_sidno() {
  ...
  if (sidno > 0) {
    ...
    PROPAGATE_REPORTED_ERROR(undeleted_gtids.ensure_sidno(sidno));
    ...
    PROPAGATE_REPORTED_ERROR(sid_locks_undeleted.ensure_index(sidno));
    ...
    assert(undeleted_gtids.get_max_sidno() >= sidno);
...
int Gtid_state::save_gtids_of_last_binlog_into_table() {
  ...
  */
  ...
  /*
    ...
    If cdb_optimize_gtid_lock=ON and opt_binlog_order_commits=OFF:
      logged_gtids_last_binlog = logged_gtids_last_binlog - owned_gtids + undeleted_gtids
  */
  global_sid_lock->wrlock();
  ret = (logged_gtids_last_binlog.add_gtid_set(&executed_gtids) !=
         RETURN_STATUS_OK);
  if (!ret) {
    /* This may happen when cdb_optimize_gtid_lock=ON and opt_binlog_order_commits=OFF. */
    if (cdb_optimize_gtid_lock && owned_gtids.is_intersection_nonempty(&executed_gtids)) {
      owned_gtids.weed_out_gtids(logged_gtids_last_binlog);
      int ret2 = (logged_gtids_last_binlog.add_gtid_set(&undeleted_gtids) != 
          RETURN_STATUS_OK);
      if (ret2) {
          global_sid_lock->unlock();
          return ret2;
...
void Gtid_state::handle_gtid_on_finish(THD *thd, bool is_commit) {
  DBUG_TRACE;
  ...
  if (is_commit) {
    ...
    undeleted_gtids._add_gtid(thd->owned_gtid);
    unlock_sidno_undeleted(sidno);
...
void Gtid_state::cleanup_owned_gtids() {
  ...
  for (iter = gtid_intervals.begin(); iter != gtid_intervals.end(); iter++) {
    if (!has_locked_sidno) {
      locked_sidno = iter->sidno;
      gtid_state->lock_sidno(locked_sidno);
      has_locked_sidno = true;
    } else if (iter->sidno != locked_sidno) {
      gtid_state->unlock_sidno(locked_sidno);
      gtid_state->lock_sidno(iter->sidno);
      locked_sidno = iter->sidno;
    }
    owned_gtids.remove_gtid_interval(iter->sidno, iter->gno_start, iter->gno_end);
  }
  if (has_locked_sidno) {
    gtid_state->unlock_sidno(locked_sidno);
  }
