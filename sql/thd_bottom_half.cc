...
*/
bool CThdBottomHalf::remove_thd(const THD *thd, enum enum_server_command &command)
{
  // Can only remove a thd that is ever given to us here.
  assert(thd->m_asyncAns);
  CTGuard<CTMutex> gaurd(m_mutex);
  ...
  */
  ...
  if (connection_should_abort(connection)) {
    if (!the_thd->m_delay_commit) {
      ...
      sql_print_error("commit delay commit connection before abort");
    }
  }
