...
static Reload_entries reload_entries;
...
SHOW_VAR reload_entry_status[] = {
    {"statement_outline",
     (char *)offsetof(System_status_var,
                      reload_stat[(uint)RELOAD_STATEMENT_OUTLINE]),
...
/**
  ...
  @retval         entry     Reload entry
*/
const Reload *lookup_reload_entry(
    const char *db, std::size_t db_len MY_ATTRIBUTE((unused)),
    const char *table_name, std::size_t table_name_len MY_ATTRIBUTE((unused))) {
  std::stringstream ss;
...
*/
void execute_reload_on_slave(THD *thd, Relay_log_info *rli) {
  assert(thd->slave_thread && rli);
  if (thd->is_error()) return;

  Reload **entry = rli->reload_entries;
  /* Ignore the error */
  Reload_error_handler error_handler(thd);
  for (int i = 0; i < RELOAD_ENTRY_COUNT; i++, entry++) {
    if (*entry) {
      (*entry)->execute(thd);
      /* Aggregate status */
      thd->status_var.reload_stat[i]++;
      rli->reload_entries[i] = nullptr;
    }
  }
}
...
*/
void mark_rli_trx_reload(THD *thd, Relay_log_info *rli, TABLE_SHARE *table_share) {
  const Reload *entry = nullptr;
  /* Require slave thread */
  assert(thd->slave_thread && rli && table_share && table_share->reload_entry);
  entry = table_share->reload_entry;
  Reload_type type = entry->type();
  rli->reload_entries[type] = const_cast<Reload *>(entry);
}
