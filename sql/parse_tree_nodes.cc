...
Sql_cmd *PT_analyze_table_stmt::make_cmd(THD *thd) {
  ...
  auto cmd = new (thd->mem_root) Sql_cmd_analyze_table(
      thd, &m_alter_info, m_command, m_num_buckets, m_data, m_version);
...
bool PT_table_factor_table_ident::contextualize(Parse_context *pc) {
  ...
  value = pc->select->add_table_to_list(
      ...
      opt_key_definition, opt_use_partition, opt_table_sample, nullptr, pc,
      opt_low_limit_timestamp, opt_up_limit_timestamp);
  if (value == nullptr) return true;
  if (pc->select->add_joined_table(value)) return true;
  return false;
}
