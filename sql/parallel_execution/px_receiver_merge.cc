...
PX_receiver_merge::PX_receiver_merge(THD *thd, uint receiver_no, PX_exchange_info *pei,
    TABLE *table, Filesort *sort, unique_ptr_destroy_only<RowIterator> source,
    JOIN *join, int ref_slice)
    : PX_receiver(thd, receiver_no, pei, join, move(source), table, ref_slice),
      m_join(join),
...
bool PX_receiver_merge::attach() {
  if (PX_receiver::attach()) return true;
  return false;
}
