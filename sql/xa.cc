...
#include "sql/item_func.h" //doMurmurHashCode
  ...
  if (hton->state == SHOW_OPTION_YES && hton->recover) {
    while (
      ...
      for (int i = 0; i < got; i++) {
        ...
        {
          const XID &target_xid = info->list[i].id;
...
int ha_recover(const memroot_unordered_set<my_xid> *commit_list,
    const std::set<std::string> *xa_prepared,
    const std::set<std::string> *xa_cop,
  ...
  if (g_log_prepared_xid_list) {
    info.binlog_xa_prepared = xa_prepared;
    info.binlog_xa_cop = xa_cop;
    info.binlog_xa_committed = xa_committed;
    info.binlog_xa_aborted = xa_aborted;
  } else {
    info.binlog_xa_prepared = info.binlog_xa_cop
                            = info.binlog_xa_committed
                            = info.binlog_xa_aborted = nullptr;
  }
...
bool Sql_cmd_xa_rollback::trans_xa_rollback(THD *thd) {
  ...
  /* Inform clone handler of XA operation. */
  Clone_handler::XA_Operation xa_guard(thd);
  if (!xid_state->has_same_xid(m_xid) && !m_force) {
...
bool Sql_cmd_xa_rollback::process_internal_xa_rollback(THD *thd,
  ...
  if(unlikely(m_force)){
  	...
  	}else if( XID_STATE::XA_ACTIVE == xid_state->get_state()) {//active ,need end
      xid_state->set_state(XID_STATE::XA_IDLE);
      MYSQL_SET_TRANSACTION_XA_STATE(thd->m_transaction_psi,
                                       (int)xid_state->get_state());
  	}
  	thd->rpl_partial_xa_rollback(true);//if it is injected by rpl thread,will remove the gtid
  }

  DBUG_ASSERT(xid_state->has_same_xid(m_xid));
...
bool Sql_cmd_xa_rollback::execute(THD *thd) {
  ...
  std::string xa_txnid(get_xa_txnid(m_xid));
  if(m_force) {
    if(!xa_txnid.empty()) {//if set force flag,the xid must is empty
      my_printf_error(ER_XAER_INVAL,"xa rollback with force,the xid[%s] must empty",MYF(0),xa_txnid.c_str());
      return true;
    }
...
bool Sql_cmd_xa_rollback::clean_state_from_coord_inject(THD *thd) {
...
bool Sql_cmd_xa_prepare::execute(THD *thd) {
  ...
  /*
     For unknown reasons, in tests we saw a xa txn prepared on binlog file but
     not written in next binlog's XA-PREPARED-LIST event. It might be caused by
     some error that happened after the binlog write( and innodb prepare), so we
     choose to always log an xa txnid, and if an xa txn prepare fails and logged
     into next binlog file's XA-PREPARED-LIST, it's almost harmless, except
     making the XA-PREPARED-LIST a bit longer.
  */
  if (g_log_prepared_xid_list) {
    std::string xa_txnid(get_xa_txnid(m_xid));
  ...
  /*
    We shouldn't allow binlog rotating before adding xid to prepared_xa_txnids.
    Otherwise, it may logg XA Prepare to binlog first, then rotate before add_id, the
    XA_PREPARED_LIST will miss the m_xid here. So we increase prep_xids for XA
    PREPARE and decrease it after invoking add_id().
  */
...
void Prepared_xa_txnids::init(uint32_t parts) {
  m_parts = parts;
  m_instances = new Xa_txnids_instance[parts];
}

uint32_t Prepared_xa_txnids::get_instance(const std::string &id) {
  if (m_parts == 1) {
    return 1;
  }

  return (doMurmurHashCode(const_cast<char*>(id.c_str()), id.length()) % m_parts);
}

void Prepared_xa_txnids::add_id(const std::string &id) {
  m_instances[get_instance(id)].add_id(id);
}

void Prepared_xa_txnids::del_id(const std::string &id) {
  m_instances[get_instance(id)].del_id(id);
}
...
int Prepared_xa_txnids::parse(const char *str, Txnids_t &ids)
{
  ...
  /*
    To be backward compatible for existing users, allow both 'xa-id-list' and
    xa-id-list form.
  */
  char *x = strchr(p, '\'');
  if (x)
  ...
  if ((!y && x) || (y && !x))
    return -1;
  if (y)
    *y = '\0'; /* temporarily clear it to get the correct last xa txnid. */
...
void Prepared_xa_txnids::from_recovery(Txnids_t &prepared, const Txnids_t &committed,
    const Txnids_t &aborted)
{
  ...
  for (Txnids_t::iterator i= prepared.begin(); i != prepared.end(); ++i) {
    add_id(*i);
  }
}
