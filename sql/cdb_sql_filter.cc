...
const char *cdb_sql_filter::Rule_Type_String[]= {
  "SELECT",
  "UPDATE",
  "INSERT",
  "DELETE",
...
static int string_to_int(char *str) 
{
  ...
  int ret= atoi(str);
  sprintf(buf, "%d", ret);
  if (strncmp(str, buf, strlen(buf)) != 0)
    return 0;
  if (ret < -1)
    return 0;
  return ret;
...
Rule::Rule(char* rule_string, int new_id, Rule_Type new_type,
  ...
  :id(new_id), concurrence(new_concurrence), current_conn(0),
  rejected_sql_count(0), expired(false),
  origin_rule_str(new_origin_rule_str), type(new_type)
{
...
bool Rule::handle_sql_enter_low(const char* sql, bool &matched)
{
  ...
  for (uint i= 0; i < key_words.size(); i++)
  {
    if (strstr(sql, key_words[i]) == NULL)
      return false;
  }
...
const char* Cdb_Sql_Filter_Manager::get_errmsg()
{
  return errmsg;
}

void Cdb_Sql_Filter_Manager::init()
{
  mysql_rwlock_init(m_key_lock, &m_lock);
}
...
bool Cdb_Sql_Filter_Manager::reset_all_rules(bool is_clean_up)
{
  ...
  {
    for (iter= filter_rules[i].begin(); iter != filter_rules[i].end(); iter++)
    {
      if (iter->second) 
      {
        if (is_clean_up)
          assert(iter->second->current_conn == 0);
        delete iter->second;
      }
    }
...
bool Cdb_Sql_Filter_Manager::handle_sql_enter(const char *sql, enum_sql_command sql_cmd,
                                          std::vector<Rule*> &v)
{
...
bool Cdb_Sql_Filter_Manager::handle_sql_exit(std::vector<Rule*> &v)
{
  ...
  for (iter= v.begin(); iter != v.end(); iter++)
    if (*iter)
      (*iter)->handle_sql_exit_low();
  unlock();
  return false;
}

bool Cdb_Sql_Filter_Manager::handle_delete_rule(char *rule)
{
...
bool Cdb_Sql_Filter_Manager::handle_add_rule(char *rule)
{
  ...
  */
  ...
  {
    errmsg= "sql_command should be [ SELECT | UPDATE | INSERT | DELETE | REPLACE ]"
  ...
  Rule *new_rule= new Rule(rule, new_rule_id, rule_type,
                            expire_time, concurrence, origin_rule_str);
...
bool Cdb_Sql_Filter_Manager::handle_rule()
{
  char *rule= cdb_sql_filter;
  if (rule == NULL || strlen(rule) == 0)
    return true;
  if (rule[0] == '+')
    return handle_add_rule(rule + 1);
  else if (rule[0] == '-')
    return handle_delete_rule(rule + 1);
...
void Cdb_Sql_Filter_Manager::get_all_rules_for_display(std::vector<display_result> &ret)
{
  std::map<int, Rule*>::iterator iter;
