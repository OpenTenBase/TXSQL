...
/**
  Preparation the connection from per_thread mode for threadpool_process_request
  Modified from threadpool_add_connection.
  @return true if vio has data, and it has to be processed.
  @return false if no vio has to be processed.
*/
bool threadpool_process_request_prepare(THD *thd) {
  ...
  /* If get killed here, we delay the handling to threadpool_process_request. */
  thd_set_net_read_write(thd, 1);
  thd->skip_wait_timeout = true;
  threadpool_init_net_server_extension(thd);
  MYSQL_SOCKET_SET_STATE(thd->get_protocol_classic()->get_vio()->mysql_socket,
  ...
  if (!vio->has_data(vio)) {
    ...
    /* NO data to read, skip threadpool_process_request this time. */
    return false;
  }
...
int threadpool_process_request(THD *thd) {
  ...
  /*
    ...
    thread-per-connections logic, see do_handle_one_connection() in
    sql_connect.cc
