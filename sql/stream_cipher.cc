...
std::unique_ptr<Stream_cipher> Sm4_ctr::get_encryptor() {
  std::unique_ptr<Stream_cipher> cipher(new Sm4_ctr_encryptor);
  return cipher;
}

std::unique_ptr<Stream_cipher> Sm4_ctr::get_decryptor() {
  std::unique_ptr<Stream_cipher> cipher(new Sm4_ctr_decryptor);
  return cipher;
}

template <Cipher_type TYPE>
bool Sm4_ctr_cipher<TYPE>::open(const Key_string &password, int header_size) {
  m_header_size = header_size;

  if (EVP_BytesToKey(Sm4_ctr::get_evp_cipher(), Sm4_ctr::get_evp_md(), nullptr,
                     password.data(), password.length(), 1, m_file_key,
                     m_iv) == 0)
    return true;

  /*
    SM4-CTR counter is set to 0. Data stream is always encrypted beginning with
    counter 0.
  */
  return init_cipher(0);
}
...
template <Cipher_type TYPE>
Sm4_ctr_cipher<TYPE>::~Sm4_ctr_cipher<TYPE>() {
  close();
}

template <Cipher_type TYPE>
void Sm4_ctr_cipher<TYPE>::close() {
  deinit_cipher();
}

template <Cipher_type TYPE>
bool Sm4_ctr_cipher<TYPE>::set_stream_offset(uint64_t offset) {
  ...
  */
  if (TYPE == Cipher_type::ENCRYPT)
    return encrypt(buffer, buffer, offset % SM4_BLOCK_SIZE);
  else
    return decrypt(buffer, buffer, offset % SM4_BLOCK_SIZE);
}

template <Cipher_type TYPE>
bool Sm4_ctr_cipher<TYPE>::init_cipher(uint64_t offset) {
  DBUG_TRACE;

  uint64_t counter = offset / SM4_BLOCK_SIZE;
  ...
  */
  ...
  int res;
  const EVP_CIPHER *cipher_type = Sm4_ctr::get_evp_cipher();
...
template <Cipher_type TYPE>
void Sm4_ctr_cipher<TYPE>::deinit_cipher() {
  if (m_ctx) EVP_CIPHER_CTX_free(m_ctx);
  m_ctx = nullptr;
}

template <Cipher_type TYPE>
bool Sm4_ctr_cipher<TYPE>::encrypt(unsigned char *dest,
                                   const unsigned char *src, int length) {
  DBUG_TRACE;
...
template <Cipher_type TYPE>
bool Sm4_ctr_cipher<TYPE>::decrypt(unsigned char *dest,
                                   const unsigned char *src, int length) {
  DBUG_TRACE;
...
template class Sm4_ctr_cipher<Cipher_type::ENCRYPT>;
template class Sm4_ctr_cipher<Cipher_type::DECRYPT>;
